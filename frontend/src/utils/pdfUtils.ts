...
export const gerarReciboPDF = (venda: Venda, idioma: IdiomaSuportado = 'pt') => {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; padding: 24px; max-width: 600px; margin: auto;">
      <h2 style="text-align: center; color: #336021;">${texto.recibo || 'Recibo de Venda'}</h2>
      ...
      <table style="width: 100%; font-size: 13px; border-collapse: collapse; margin-top: 15px;">
        <thead>
          <tr>
            <th style="border-bottom: 1px solid #ccc; text-align: left;">${texto.produto || 'Produto'}</th>
            <th style="border-bottom: 1px solid #ccc; text-align: center;">${texto.qtd || 'Qtd'}</th>
            <th style="border-bottom: 1px solid #ccc; text-align: center;">${texto.unitario || 'Unitário'}</th>
            <th style="border-bottom: 1px solid #ccc; text-align: center;">${texto.total || 'Total'}</th>
          </tr>
        </thead>
        <tbody>
          ...
          `).join('')}
          <tr>
            <td colspan="3" style="text-align: right; font-weight: bold; padding-top: 8px;">${texto.subtotal || 'Subtotal'}</td>
            <td style="text-align: center;">R$ ${Number(subtotal).toFixed(2)}</td>
          </tr>
          ${Number(desconto_aplicado) > 0 ? `
            <tr>
              <td colspan="3" style="text-align: right; color: red;">${texto.desconto || 'Desconto'}</td>
              <td style="text-align: center; color: red;">- R$ ${Number(desconto_aplicado).toFixed(2)}</td>
            </tr>
          ` : ''}
          <tr>
            <td colspan="3" style="text-align: right; font-weight: bold;">${texto.totalFinal || 'Total Final'}</td>
